...
class NextDetector {
    async detect(files, workspaceRoot) {
        ...
        if (rootFiles.includes('next.config.js') || rootFiles.includes('next.config.ts') || rootFiles.includes('next.config.mjs')) {
            return { detected: true, technology: 'Next.js', category: constants_1.CATEGORIES.NEXT, confidence: 'high' };
        }
        try {
            ...
            return { detected, technology: 'Next.js', category: constants_1.CATEGORIES.NEXT, confidence: detected ? 'high' : 'low' };
        }
        catch {
            return { detected: false, technology: 'Next.js', category: constants_1.CATEGORIES.NEXT, confidence: 'low' };
        }
    }
}
